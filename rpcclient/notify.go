...
type notificationState struct {
	...
	notifySpent        map[acbcjson.OutPoint]struct{}
}
...
func (s *notificationState) Copy() *notificationState {
	...
	stateCopy.notifySpent = make(map[acbcjson.OutPoint]struct{})
...
func newNotificationState() *notificationState {
	return &notificationState{
		...
		notifySpent:    make(map[acbcjson.OutPoint]struct{}),
	}
}
...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *acbcutil.Tx, details *acbcjson.BlockDetails)
	...
	//
	// Deprecated: Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *acbcutil.Tx, details *acbcjson.BlockDetails)
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount acbcutil.Amount)

	// OnTxAccepted is invoked when a transaction is accepted into the
	// memory pool.  It will only be invoked if a preceding call to
	// NotifyNewTransactions with the verbose flag set to true has been
	// made to register for the notification and the function is non-nil.
	OnTxAcceptedVerbose func(txDetails *acbcjson.TxRawResult)
	...
	// such as btcwallet.
	OnAccountBalance func(account string, balance acbcutil.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	switch ntfn.Method {
	// OnBlockConnected
	case acbcjson.BlockConnectedNtfnMethod:
	...
	case acbcjson.FilteredBlockConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBlockDisconnected
	case acbcjson.BlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case acbcjson.FilteredBlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRecvTx
	case acbcjson.RecvTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRedeemingTx
	case acbcjson.RedeemingTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case acbcjson.RelevantTxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanFinished
	case acbcjson.RescanFinishedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanProgress
	case acbcjson.RescanProgressNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAccepted
	case acbcjson.TxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAcceptedVerbose
	case acbcjson.TxAcceptedVerboseNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBtcdConnected
	case acbcjson.BtcdConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnAccountBalance
	case acbcjson.AccountBalanceNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case acbcjson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*acbcutil.Tx, error) {
	...
	transactions := make([]*acbcutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = acbcutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*acbcutil.Tx,
	*acbcjson.BlockDetails, error) {
	...
	// JSON object.
	var block *acbcjson.BlockDetails
	if len(params) > 1 {
	...
	return acbcutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := acbcutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*acbcjson.TxRawResult,
	error) {
	...
	var rawTx acbcjson.TxRawResult
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance acbcutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := acbcutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
func (c *Client) NotifyBlocksAsync() FutureNotifyBlocksResult {
	...
	cmd := acbcjson.NewNotifyBlocksCmd()
...
func (c *Client) notifySpentInternal(outpoints []acbcjson.OutPoint) FutureNotifySpentResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
	...
	cmd := acbcjson.NewNotifySpentCmd(outpoints)
	return c.SendCmd(cmd)
}

// newOutPointFromWire constructs the acbcjson representation of a transaction
// outpoint from the wire type.
func newOutPointFromWire(op *wire.OutPoint) acbcjson.OutPoint {
	return acbcjson.OutPoint{
		Hash:  op.Hash.String(),
		Index: op.Index,
	}
}
...
func (c *Client) NotifySpentAsync(outpoints []*wire.OutPoint) FutureNotifySpentResult {
	...
	ops := make([]acbcjson.OutPoint, 0, len(outpoints))
	for _, outpoint := range outpoints {
		ops = append(ops, newOutPointFromWire(outpoint))
	}
	cmd := acbcjson.NewNotifySpentCmd(ops)
	return c.SendCmd(cmd)
}
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := acbcjson.NewNotifyNewTransactionsCmd(&verbose)
	return c.SendCmd(cmd)
}
...
func (c *Client) notifyReceivedInternal(addresses []string) FutureNotifyReceivedResult {
	...
	cmd := acbcjson.NewNotifyReceivedCmd(addresses)
	return c.SendCmd(cmd)
}
...
//
// Deprecated: Use LoadTxFilterAsync instead.
func (c *Client) NotifyReceivedAsync(addresses []acbcutil.Address) FutureNotifyReceivedResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
	...
	cmd := acbcjson.NewNotifyReceivedCmd(addrs)
	return c.SendCmd(cmd)
}
...
//
// Deprecated: Use LoadTxFilter instead.
func (c *Client) NotifyReceived(addresses []acbcutil.Address) error {
...
func (c *Client) RescanAsync(startBlock *chainhash.Hash,
	...
	cmd := acbcjson.NewRescanCmd(startBlockHashStr, addrs, ops, nil)
	return c.SendCmd(cmd)
}
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	addresses []acbcutil.Address, outpoints []*wire.OutPoint,
	...
	cmd := acbcjson.NewRescanCmd(startBlockHashStr, addrs, ops,
		&endBlockHashStr)
	return c.SendCmd(cmd)
}
...
// NOTE: This is a btcd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilterAsync(reload bool, addresses []acbcutil.Address,
	...
	outPointObjects := make([]acbcjson.OutPoint, len(outPoints))
	for i := range outPoints {
		outPointObjects[i] = acbcjson.OutPoint{
			Hash:  outPoints[i].Hash.String(),
			Index: outPoints[i].Index,
		}
	}

	cmd := acbcjson.NewLoadTxFilterCmd(reload, addrStrs, outPointObjects)
	return c.SendCmd(cmd)
}
...
// NOTE: This is a btcd extension ported from github.com/decred/dcrrpcclient
// and requires a websocket connection.
func (c *Client) LoadTxFilter(reload bool, addresses []acbcutil.Address, outPoints []wire.OutPoint) error {
