...
import (
	...
	_ "github.com/MonteCarloClub/acbc/database/ffldb"
...
var (
	...
	// todo：
	knownDbTypes       = []string{"levelDB", "mongoDB", "ffldb"}
...
type config struct {
	...
	AgentBlacklist       []string      `long:"agentblacklist" description:"A comma separated list of user-agent substrings which will cause github.com/MonteCarloClub/acbc to reject any peers whose user-agent contains any of the blacklisted substrings."`
	AgentWhitelist       []string      `long:"agentwhitelist" description:"A comma separated list of user-agent substrings which will cause github.com/MonteCarloClub/acbc to require all peers' user-agents to contain one of the whitelisted substrings. The blacklist is applied before the blacklist, and an empty whitelist will allow all agents that do not fail the blacklist."`
...
// validDbType returns whether dbType is a supported database type.
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableRPC {
		acbcLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = acbcutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	/*
		...
		cfg.miningAddrs = make([]acbcutil.Address, 0, len(cfg.MiningAddrs))
		for _, strAddr := range cfg.MiningAddrs {
			addr, err := acbcutil.DecodeAddress(strAddr, activeNetParams.Params)
			if err != nil {
	...
	*/
	...
	if configFileError != nil {
		acbcLog.Warnf("%v", configFileError)
	}
...
// github.com/MonteCarloClub/acbcDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func acbcDial(addr net.Addr) (net.Conn, error) {
...
// github.com/MonteCarloClub/acbcLookup resolves the IP of the given host using the correct DNS lookup
...
func acbcLookup(host string) ([]net.IP, error) {
